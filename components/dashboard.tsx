...
const adaptationChips = ['Make it cheaper', 'Less walking'];
      ...
      <div className='relative z-10 bg-neutral-900/90 border border-neutral-800/50 rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl shadow-black/50'>
        ...
        <div className='text-center mb-6'>
          ...
          <p className='text-sm text-neutral-500'>
            Your personalized trip briefing
          </p>
        </div>
...
function MobileIntelligencePanel({
  ...
  budget: {
    total: number;
    spent: number;
    breakdown: { food: number; activity: number; travel: number };
    currency: string;
  };
  onReset: () => void;
    ...
    if (response) {
      ...
      if (tripJson) {
        ...
        if (mappedData && isValidTripData(mappedData)) {
          ...
          setAiResponse(
            response.reply ||
              'Received a trip update but the format was invalid.',
          );
        }
      } else {
            ...
            <>
              ...
              <div className='bg-neutral-900/60 rounded-lg p-3'>
                <div className='flex items-start gap-2'>
                  ...
                  <div className='flex-1 min-w-0'>
                    {loading && lastCommand && (
                      <p className='text-xs text-neutral-500 italic mb-1 truncate'>
                        {lastCommand}
                      </p>
                    )}
                    {loading ? (
                      ...
                      <p className='text-xs text-neutral-400 leading-relaxed line-clamp-3'>
                        {aiResponse ||
                          'Send a command to interact with Trip OS...'}
                      </p>
                    )}
                    {error && (
              ...
              <form onSubmit={handleCommand} className='relative'>
                <input
                  ...
                  placeholder={
                    !isReady ? 'Connecting...' : 'Command Trip OS...'
                  }
                ...
                <button
                  ...
                  className='absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 hover:bg-emerald-500/30 transition-all disabled:opacity-50 disabled:cursor-default'
                >
                  {loading ? (
...
export function Dashboard({
  ...
  const currentDay =
    tripData.days.find((d) => d.day === selectedDay) || tripData.days[0];
  ...
  const totalSpent =
    budgetBreakdown.food + budgetBreakdown.activity + budgetBreakdown.travel;
    ...
    <div className='min-h-screen flex flex-col bg-neutral-950 text-neutral-200'>
      ...
      <header className='min-h-12 py-2 px-4 flex flex-wrap items-center justify-center gap-3 sm:gap-6 relative z-10 bg-neutral-950/80 backdrop-blur-sm border-b border-neutral-800/30 animate-fade-in'>
        ...
        <Link href='/' className='flex items-center gap-2 group cursor-pointer'>
          ...
          <span className='text-sm font-medium text-neutral-200 group-hover:text-white transition-colors duration-300'>
            Trip OS
          </span>
        </Link>
        ...
        >
          ...
          <span className='hidden xs:inline'>
            {isBriefingLoading ? 'Generating...' : 'Briefing'}
          </span>
        </button>
        ...
        <div className='flex items-center gap-2 group cursor-pointer'>
          ...
          <span className='text-sm text-neutral-200 group-hover:text-white transition-colors duration-300 max-w-[120px] sm:max-w-none truncate'>
            {tripData.destination}
          </span>
        </div>
      </header>
