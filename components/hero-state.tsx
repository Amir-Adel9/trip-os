...
const exampleTrips = [
  ...
  },
  {
    ...
    prompt:
      '5 days in Dubai experiencing luxury, Burj Khalifa and desert safari',
...
export function HeroState({ onSubmit }: HeroStateProps) {
  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center px-6 py-12 relative overflow-hidden'>
      ...
      <div
        className='absolute bottom-1/4 right-[15%] w-[400px] h-[400px] bg-violet-500/[0.03] rounded-full blur-[100px] animate-breathe'
      ...
      <div className='relative z-10 flex flex-col items-center w-full max-w-2xl'>
        ...
        >
          <div
            className={`relative group ${isFocused ? 'animate-glow-pulse' : ''}`}
          >
            {/* Input Glow Effect */}
            <div
              className={`absolute -inset-px rounded-2xl bg-gradient-to-r from-emerald-500/50 via-teal-500/50 to-emerald-500/50 opacity-0 blur-sm transition-opacity duration-500 ${isFocused ? 'opacity-100' : 'group-hover:opacity-50'}`}
            />
            ...
            <div className='relative bg-neutral-900/80 backdrop-blur-xl rounded-2xl border border-neutral-800/80 overflow-hidden transition-all duration-300 group-hover:border-neutral-700/80'>
              <div className='flex items-center px-5 py-4'>
                <Sparkles
                  className={`w-5 h-5 mr-4 transition-colors duration-300 ${isFocused ? 'text-emerald-400' : 'text-neutral-600'}`}
                />
                <input
                  type='text'
                  value={prompt}
                  ...
                  className='flex-1 bg-transparent text-lg text-white placeholder:text-neutral-600 outline-none transition-all duration-300 disabled:opacity-50 disabled:cursor-default'
                />
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-3 w-full'>
          {exampleTrips.map((trip, idx) => (
            ...
            >
              ...
              <div
                className={`relative shrink-0 w-11 h-11 rounded-xl ${trip.bg} flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:rotate-3`}
              >
                <trip.icon
                  className={`w-5 h-5 ${trip.color} transition-all duration-300 group-hover:scale-110`}
                />
              </div>
